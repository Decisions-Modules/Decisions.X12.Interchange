...
[AutoRegisterMethodsOnClass(true, "Data", "X12", "945")]
public static class X12Steps945
{
    public static Interchange Deserialize945EDI(string document945, bool inputIsPath = false)
    {
        // EDI string -> X12 Xml string.
        Decisions.X12.Parsing.Model.Interchange interchange;

        using (FileStream fs = inputIsPath
                   ? new FileStream(document945, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 4096, FileOptions.None)
                   ...
                   FileShare.None, 4096, FileOptions.DeleteOnClose))
        {
                       ...
                       new XmlReaderSettings { IgnoreComments = true, CheckCharacters = false }))
            {
                ...
                if (result?.FunctionGroup?.Transaction?.ST?.ST01 != "945")
                    throw new InvalidOperationException("Incorrect document being used. Please use 945");

                Transaction945? tx = result?.FunctionGroup?.Transaction;
    ...
    private static void HandleUnknownElement(object obj, XmlElementEventArgs args)
    {
        ...
        {
            case "0100": // NameAddressLoop945
            {
                Transaction945? transaction = args?.ObjectBeingDeserialized as Transaction945;
                if (transaction == null)
                    break;
                NameAddressLoop945 loop = GetLoopValue<NameAddressLoop945>(args.Element);
                if (transaction.NameAddressLoopForDeserialize == null)
                    transaction.NameAddressLoopForDeserialize = new List<NameAddressLoop945>();
                transaction.NameAddressLoopForDeserialize.Add(loop);
            }
                break;
            case "0300": // AssignedNumberLoop945
            {
                Transaction945? transaction = args?.ObjectBeingDeserialized as Transaction945;
                if (transaction == null)
                    break;
                AssignedNumberLoop945 loop = GetLoopValue<AssignedNumberLoop945>(args.Element);
                if (transaction.AssignedNumberLoopForDeserialize == null)
                    transaction.AssignedNumberLoopForDeserialize = new List<AssignedNumberLoop945>();
